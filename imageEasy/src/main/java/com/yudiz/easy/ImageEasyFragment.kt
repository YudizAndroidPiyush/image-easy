...
import com.yudiz.easy.adapters.InstantImageAdapter
import com.yudiz.easy.adapters.MainImageAdapter
import com.yudiz.easy.databinding.ControlsLayoutBinding
import com.yudiz.easy.databinding.FragmentImageeasyBinding
import com.yudiz.easy.databinding.GridLayoutBinding
import com.yudiz.easy.databinding.PermissionsLayoutBinding
import com.yudiz.easy.databinding.VideoCounterLayoutBinding
import com.yudiz.easy.helpers.*
import com.yudiz.easy.interfaces.OnSelectionListener
import com.yudiz.easy.models.Img
import com.yudiz.easy.models.Options
import com.yudiz.easy.models.ImageEasyViewModel
import com.yudiz.easy.utility.ARG_PARAM
import com.yudiz.easy.utility.ARG_PARAM_KEY
import com.yudiz.easy.utility.CustomItemTouchListener
...
class ImageEasyFragment(private val resultCallback: ((ImageEasyEventCallback.Results) -> Unit)? = null) :
    Fragment(), View.OnTouchListener {

    private val model: ImageEasyViewModel by viewModels()
    private var _binding: ImageEasyBindings? = null
        ...
        if (mBottomSheetBehavior?.state == BottomSheetBehavior.STATE_COLLAPSED) {
            Handler(Looper.getMainLooper()).postDelayed({
                try {
                    ...
                    e.message?.let { Log.e("ImageEasyFragment", it) }
                }
            }, 200)
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        options = arguments?.getParcelable(ARG_PARAM) ?: Options()
        requireActivity().let {
            it.setupScreen()
            it.actionBar?.hide()
            colorPrimaryDark = it.color(com.yudiz.easy.R.color.primary_color)
        }
    }
    ...
    override fun onCreateView(
        ...
        val fragmentImageEasyBinding = FragmentImageeasyBinding.inflate(inflater, container, false)
        _binding = ImageEasyBindings(
            fragmentImageEasyBinding,
            VideoCounterLayoutBinding.bind(fragmentImageEasyBinding.root),
            PermissionsLayoutBinding.bind(fragmentImageEasyBinding.root),
            GridLayoutBinding.bind(fragmentImageEasyBinding.root),
            ControlsLayoutBinding.bind(fragmentImageEasyBinding.root)
        )
        fragmentImageEasyBinding.root
    }
    ...
    private fun FragmentActivity.setup() {
        ...
        setFragmentResultListener(ARG_PARAM_KEY) { _, bundle ->
            val options1: Options? = bundle.getParcelable(ARG_PARAM)
            options1?.let {
                this@ImageEasyFragment.options.preSelectedUrls.apply {
                    clear()
    ...
    private fun initialise(context: FragmentActivity) {
        ...
        binding.gridLayout.gridLayout.show()
        cameraXManager =
            CameraXManager(binding.fragmentImageEasy.viewFinder, context, options).also {
                it.startCamera()
            }
    ...
    private fun observeSelectionList() {
        ...
        model.callResults.observe(requireActivity()) { event ->
            event?.getContentIfNotHandledOrReturnNull()?.let { set ->
                ...
                resultCallback?.invoke(ImageEasyEventCallback.Results(results))
                ImageEasyBus.returnObjects(
                    event = ImageEasyEventCallback.Results(
                        results,
    ...
    private fun setupFastScroller(context: FragmentActivity) {
        ...
        binding.gridLayout.apply {
            ...
            fastscrollHandle.setOnTouchListener(this@ImageEasyFragment)
        }
    }
    ...
    private fun setupAdapters(context: FragmentActivity) {
        ...
        binding.gridLayout.apply {
            ...
            recyclerView.setupMainRecyclerView(
                context, mainImageAdapter, scrollListener(this@ImageEasyFragment, binding)
            )
        }
    }
