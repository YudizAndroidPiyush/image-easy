...
import com.yudiz.easy.models.Flash
import com.yudiz.easy.models.Mode
...
import com.yudiz.easy.utility.TAG
...
fun ImageEasyBindings.setDrawableIconForFlash(options: Options) {
...
internal fun ImageEasyBindings.setupClickControls(
    model: ImageEasyViewModel,
    ...
    controlsLayout.messageBottom.setText(
        when (options.mode) {
            Mode.Picture -> R.string.bottom_message_without_video
            Mode.Video -> R.string.imageeasy_bottom_message_with_only_video
            else -> R.string.bottom_message_with_video
        }
    )
    controlsLayout.primaryClickButton.apply {
        ...
        setOnTouchListener { _, event ->
            if (event.action == MotionEvent.ACTION_UP || event.action == MotionEvent.ACTION_CANCEL) {
                ...
                fragmentImageEasy.root.requestDisallowInterceptTouchEvent(false)
                ...
                fragmentImageEasy.root.requestDisallowInterceptTouchEvent(true)
            }
...
fun ImageEasyBindings.longSelectionStatus(
    enabled: Boolean
) {
    val colorPrimaryDark = fragmentImageEasy.root.context.color(R.color.primary_color)
    val colorSurface = fragmentImageEasy.root.context.color(R.color.surface_color)

    if (enabled) {
        gridLayout.selectionCheck.hide()
        gridLayout.selectionCount.setTextColor(R.color.surface_color)
...
fun ImageEasyBindings.setSelectionText(fragmentActivity: FragmentActivity, size: Int = 0) {
    gridLayout.selectionCount.text = if (size == 0) {
        gridLayout.selectionOk.hide()
        fragmentActivity.resources.getString(R.string.imageeasy_tap_to_select)
    } else {
        gridLayout.selectionOk.show()
        "$size ${fragmentActivity.resources.getString(R.string.imageeasy_selected)}"
    }
